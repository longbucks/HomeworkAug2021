...
// const cities = data.forEach(usr => console(usr.address));
// create an array of latitude and longitude coordinate pairs
// const coors = [];
// const latLog = data.forEach(usr => coors.push(usr.address.geo));
// console.log(coors);
// const coords = [];
...
const jargon = [];
const jargonPhrase = data.forEach(usr => {
  let bS = `${usr.company.bs}  ${usr.company.catchPhrase}`;
  jargon.push(bS);
});
console.log(jargon);
