...
// let comp = data.filter(usr => {
//   return usr.completed == false;
// });
// console.log(comp);

// filter for user 6's tasks

// let usr = data.filter(u => {
//   return u.userId === 6;
// });
// console.log(usr);
const userAnyTask = (uId, state) => {
  let uData = data.filter(element => {
    return element.userId === uId && element.completed == state;
  });
  console.log(uData);
};
// userAnyTask(10, false);

// filter for tasks with a title less than 24 characters long
const shortTitles = data.filter(task => {
  return task.title.length < 24;
});
