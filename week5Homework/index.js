...
let newArray = [];
let phoneArray = [];
const getPhoneNums = data.map(function(input) {
  let phoneNums = input.phone;
  let web = input.website;
  let eMail = input.email;
  newArray.push([web, eMail]);
  phoneArray.push(phoneNums);
});
...
let newArray2 = [];
const getUserNineData = Todo.map(data => {
  let usr = data.userId;
  if (usr == 9 && data.completed == false) {
    newArray2.push(data);
  }
});
...
let newArray3 = [];
const getPostsData = posts.map(data => {
  let usr = data.userId;
